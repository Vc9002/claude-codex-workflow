...
**Fetching canonical text:** canonical CC legalcodes are available as plain
text at `creativecommons.org/licenses/<code>/legalcode.txt` (e.g.
`creativecommons.org/licenses/by/4.0/legalcode.txt`) — fetch them directly
(curl or equivalent) rather than through a page-fetch/summarising tool,
which can truncate or summarise verbatim legal text instead of returning it
byte-exact.
