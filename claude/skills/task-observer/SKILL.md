...
6. If the skill first activates mid-session (the CLAUDE.md instruction
   added, or the skill invoked, partway through a task), run a condensed
   protocol instead of backfilling start-only triggers: create the
   workspace files if missing, scan OPEN observations into awareness, then
   flush accumulated observations at session end with the normal surfacing
   summary. The one-line review offer and the periodic checkpoint flushes
   are start-of-session triggers — don't attempt to retrofit them; a late
   activation that skips them is expected, not a violation.
...
2. *Pre-write assertion:* immediately before appending, confirm the proposed
   ...
   **The append itself must use this same `$PROPOSED` variable, never a
   number transcribed by hand.** A prose rule ("use the number you just
   computed") is not enough — write the actual append as one script that
   both asserts and writes the same variable, e.g.:

   ```bash
   cat >> log.md <<EOF
   ### Observation ${PROPOSED}: [title]
   ...
   EOF
   ```

   never a heredoc with a literal digit copied from earlier in the session.
   The pre-write assertion can pass (it checked the right number) while the
   body still writes a stale one, if the two aren't the same variable —
   this has happened. **Diagnostic signature:** if a post-write check shows
   the header count went up by exactly 1 but zero occurrences of the
   proposed number exist, that's this exact failure — a different, stale
   number was written instead.

   **A quoted heredoc (`<<'EOF'`, needed to prevent backtick/expansion
   execution inside the body) cannot expand shell variables at all** — so
   `${PROPOSED}` inside a quoted heredoc body is written literally, not
   substituted. This has produced a hand-typed, stale number even when the
   pre-write assertion correctly checked a fresh one. Fix: write the body
   with a literal placeholder number to a temp file, compute the real N from
   a fresh read of the log, `sed`-replace the placeholder with N, then append
   and verify — so the same computed value reaches the body even when the
   heredoc itself can't carry a variable.

   **One number per entry — never concatenate the counter with a suffix.**
   For multiple entries in one append, build titles as `"### Observation
   ${N1}:"`, `"### Observation ${N2}:"` from genuinely incremented integer
   variables — never as string concatenation like `"${PROPOSED}2:"`, which
   produces `562` instead of `57` when `PROPOSED=56`. This has happened and
   silently polluted the counter (the next writer computes past a gap of
   skipped numbers). The post-write check must validate monotonic +1
   spacing of the new entries, not merely that the highest number exists
   once — a spacing check catches the concatenation bug that a bare
