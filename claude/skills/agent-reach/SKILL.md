...
## Verify bibliographic metadata at the source before citing research

For any paper or source cited with numbers in a research deliverable, don't
trust a publisher-page or full-text fetch at face value — a page-fetch tool
can serve a mismatched abstract, or return 0 bytes for a full-text PDF on
some hosts. Verify DOI-to-title/author mapping via Crossref
(`api.crossref.org/works/{doi}`) before writing it into a deliverable; it
resolves authoritatively in one call. When a full-text fetch fails, switch
source (IDEAS/RePEc, an arXiv abstract page, an institutional repo) rather
than dropping the citation or skipping verification — a failed fetch is a
signal to re-source, not to skip checking.

**Principle:** Bibliographic claims are only as good as the metadata source
they were verified against; a failed fetch is a signal to re-source, not to
skip checking.
