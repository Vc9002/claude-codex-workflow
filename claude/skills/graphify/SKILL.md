...
**Discovery scope & project root validation:** Before initiating codebase discovery or graph building, resolve and validate the target root directory. **Refuse whole-home directory scanning (`~`, `/Users/username`, `/home/username`) or filesystem roots (`/`) by default.** An agent's convenient working directory is not authorization to scan the user's entire home tree. If invoked from a user's home directory without a specific project path, instruct the user to specify a repository path (or `cd` into the project). Never scan hidden directories (`.*`) or traverse symlinks across directory boundaries unless explicitly requested.

If no path was given and the current working directory is a specific project root, use `.` (current directory). Do not ask the user for a path when already inside a valid project.
